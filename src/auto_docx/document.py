...
class DocumentGenerator:
    ...
    def _cleanup_images(self, result: ExecutionResult) -> None:
        """Remove the temporary images directory after document generation."""
        images_dir = result.script_path.parent / f".{result.script_path.stem}_images"
    ...
    def _add_error_section(self, doc: Document, result: ExecutionResult) -> None:
        """Add stderr/error output section."""
        doc.add_heading("Errors", level=1)
        
        if result.error_message:
            error_para = doc.add_paragraph()
            error_run = error_para.add_run(f"Execution Error: {result.error_message}")
            error_run.font.color.rgb = RGBColor(255, 0, 0)
            error_run.font.bold = True
        
        if result.stderr.strip():
            self._add_code_block(doc, result.stderr, is_error=True)
        elif not result.error_message:
            doc.add_paragraph("(No error output)")
    ...
    def _add_code_block(
        ...
        Args:
            ...
            code: The code/text to add
            is_error: Whether this is error output (uses red color)
        """
    ...
    def _add_output_items(self, doc: Document, items: list) -> None:
        ...
        for item in items:
            ...
            if kind == "image":
                ...
                # Add markdown content as regular paragraph
